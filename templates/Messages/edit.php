<p class="mb-4">
    <a href="/messages/view/<?= $message->id ?>">
    <i class="bi bi-caret-left me-1"></i> Go Back to Reading Mode
    </a>
</p>
<br><br>
<h4 class="text-center mb-3" id="newreply">Message Editing Mode</h4>
<?= $this->Flash->render() ?>
<div class="row">
    <div class="card col-lg-6 mx-auto">
        <div class="card-body">
            <?= $this->Form->create($message, ['autocomplete' => 'off', 'class' => 'mb-3']) ?>
                <input autocomplete="false" name="hidden" type="text" style="display:none;">
                <input type="hidden" name="message_id" id="message_id" value="<?= $message->id ?>">
                <div class="row mb-3">                    
                    <div class="col-12">
                        <label for="subject" class="form-label">Subject</label>
                        <input type="text" class="form-control" id="subject" name="subject" autocomplete="new-text" value="<?= h($message->subject) ?>">
                        <?php if (isset($errors['subject'])) : ?><br><small class="text-danger"><?= $errors['subject'] ?></small><?php endif ?>
                    </div>
                </div>
                <div class="row mb-3">
                    <div class="col-12">
                        <label for="body" class="form-label">Message</label>
                        <textarea class="form-control" id="body" name="body" rows="15" autocomplete="new-text"><?= h($message->body) ?></textarea>
                        <?php if (isset($errors['body'])) : ?><br><small class="text-danger"><?= $errors['body'] ?></small><?php endif ?>
                    </div>
                </div>
                <div class="row mb-3">
                    <div class="col-12 text-end">
                        <!-- <button type="reset" class="btn btn-secondary">
                            <i class="bi bi-eraser-fill me-1"></i>  Reset
                        </button> -->
                        <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#exampleModal">
                            <i class="bi bi-text-left me-1"></i> Preview
                        </button>
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-check-lg me=1"></i> Post
                        </button>
                    </div>
                </div>
            <?= $this->Form->end() ?>
            <div class="modal fade" id="exampleModal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl verybigmodal">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title"> </h5>
                        </div>
                        <div class="modal-body" style="white-space: pre-wrap;">
                            
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<br><br>
<?php if ($messages->count() > 0): ?>
<h4 class="text-center mb-3" id="newpost">Messages in this Thread</h4>
<p>
    <?php foreach ($messages as $msg): ?>    
    <?= str_repeat(' &nbsp; ', 6 * $msg->level) ?> 
    <i class="bi bi-caret-right-fill text-secondary"></i>    
    <?php if ($message->id != $msg->id) : ?>
    <a href="/messages/view/<?= $msg->id ?>"><?= h($msg->subject) ?></a>
    <?php else : ?>
    <b><?= h($msg->subject) ?></b>
    <?php endif ?>
    <span class="blockquote-footer">
        by <i class="bi bi-person-fill"></i> 
        <b><?= h($msg->user->username) ?></b> 
        posted on <?= $this->Time->nice($msg->created) ?>
    </span><br>    
    <?php endforeach ?>
</p>
<?php endif ?>
<script>
$(document).ready(function() {
    $('#exampleModal').on('show.bs.modal', function (event) {
        var button = $(event.relatedTarget);
        var modal = $(this);
        modal.find('.modal-title').text($('#subject').val());
        modal.find('.modal-body').html($('#body').val());
    });
});
</script>